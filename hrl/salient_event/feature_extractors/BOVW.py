...
class BOVW(FeatureExtractor):
    ...
    def get_sift_features(self, states):
        ...
        Args:
            states (list(np.array)): list of np.array (should be iamges)
        ...
        keypoints = self.sift_detector.detect(states)
        keypoints, descriptors = self.sift_detector.compute(states, keypoints)
    ...
    def train_kmeans(self, sift_feats):
        ...
        Args:
            sift_feats (list(np.array)): list of SIFT features
        '''
        # reshape the data
        ...
        sift_feats=np.concatenate(sift_feats, axis=0)
    ...
    def histogram_from_sift(self, sift_feats):
        ...
        sift_feats_of_all_images = np.concatenate(sift_feats, axis=0)

...
class SupervisedRainbow:
    ...
    def gc_experience_replay(self, trajectory, goal, goal_position):
        ...
        def is_close(pos1, pos2, tol):
            return abs(pos1[0]-pos2[0]) <= tol and abs(pos1[0]-pos2[0]) <= tol
        ...
        for state, action, _, next_state, done, reset, next_pos in trajectory:
            ...
            relabled_transition = augmented_state, action, reward, augmented_next_state, reached or done, reset
            self.step(*relabled_transition)
            if reached:
                break # it helps to truncate the trajectory for HER strategy `future`
    ...
    def add_demonstration_trajectory(self, trajectory):
        self.agent.add_demonstration_trajectory(trajectory)
